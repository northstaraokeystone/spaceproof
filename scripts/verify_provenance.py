...
def format_results(results: dict) -> str:
    ...
    if results["hash_mismatches"]:
        ...
        for mismatch in results["hash_mismatches"]:
            lines.append(
                f"  Line {mismatch.get('line', '?')}: {mismatch.get('receipt_type', 'unknown')}"
            )
    ...
    if results["broken_chains"]:
        ...
        for chain_break in results["broken_chains"]:
            if "gap_seconds" in chain_break:
                ...
                lines.append(
                    f"  Line {chain_break['line']}: {chain_break.get('error', 'Unknown error')}"
                )
        lines.append("")
        ...
        if real_results["errors"]:
            ...
            for err in real_results["errors"]:
                if isinstance(err, dict):
                    print(
                        f"  {err.get('dataset_id', 'unknown')}: {err.get('error', 'Unknown')}"
                    )
                else:
                    print(f"  {err}")

...
grep -rq "anomaly" spaceproof/partition.py || { echo "FAIL: no anomaly detection in partition.py"; exit 1; }

# 2. Check for stoprule
grep -rq "StopRule" spaceproof/partition.py || { echo "FAIL: no StopRule in partition.py"; exit 1; }
grep -rq "stoprule\|StopRule" spaceproof/core.py || { echo "FAIL: no stoprules in core.py"; exit 1; }
...
python -c "
from spaceproof.partition import stress_sweep, NODE_BASELINE, PARTITION_MAX_TEST_PCT, BASE_ALPHA

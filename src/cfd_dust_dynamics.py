...
def load_les_config() -> Dict[str, Any]:
    ...
    emit_receipt(
        "les_config",
        {
            ...
            "smagorinsky_constant": config.get("smagorinsky_constant", LES_SMAGORINSKY_CONSTANT),
            "reynolds_threshold": config.get("reynolds_threshold", LES_REYNOLDS_THRESHOLD),
            "validated": config.get("validated", True),
            "payload_hash": dual_hash(json.dumps(config, sort_keys=True)),
        },
    )
...
def smagorinsky_viscosity(
    ...
    emit_receipt(
        "les_smagorinsky",
        {
            ...
            "payload_hash": dual_hash(json.dumps({"nu_sgs": round(nu_sgs, 6)}, sort_keys=True)),
        },
    )
...
def compute_subgrid_stress(
    ...
    emit_receipt(
        "les_stress",
        {
            ...
            "payload_hash": dual_hash(json.dumps({"tau": round(tau_sgs, 8)}, sort_keys=True)),
        },
    )
...
def simulate_les(
    ...
    v_char = reynolds * CFD_VISCOSITY_MARS_PA_S / (CFD_DENSITY_MARS_KG_M3 * filter_width)
...
def simulate_les_dust_devil(
    ...
    """
    # Validate parameters
    diameter_m = max(LES_DUST_DEVIL_DIAMETER_M[0], min(diameter_m, LES_DUST_DEVIL_DIAMETER_M[1]))
    height_m = max(LES_DUST_DEVIL_HEIGHT_M[0], min(height_m, LES_DUST_DEVIL_HEIGHT_M[1]))
    ...
    tau_wall = 0.5 * CFD_DENSITY_MARS_KG_M3 * v_tangential ** 2 * 0.01  # Friction coefficient ~0.01
    dust_lifting_capacity_kg_s = tau_wall * math.pi * (diameter_m / 2) ** 2 * 0.001  # Empirical
...
def les_vs_rans_comparison(reynolds: float) -> Dict[str, Any]:
    ...
    rans_nu_t = 0.09 * (reynolds * CFD_VISCOSITY_MARS_PA_S / CFD_DENSITY_MARS_KG_M3) ** 2 / reynolds
...
def run_les_validation() -> Dict[str, Any]:
    ...
    emit_receipt(
        "les_dust",
        {
            ...
            "validated": validated,
            "payload_hash": dual_hash(json.dumps({"validated": validated}, sort_keys=True)),
        },
    )

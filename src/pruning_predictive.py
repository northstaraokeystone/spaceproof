...
def generate_gnn_predictions(
    merkle_tree: Dict[str, Any], entropy_classifications: Dict[str, Any]
) -> List[Dict[str, Any]]:
...
def predictive_prune(
    ...
    if avg_confidence < MIN_CONFIDENCE_THRESHOLD:
        ...
        raise StopRule(
            f"Predictive confidence {avg_confidence:.3f} < {MIN_CONFIDENCE_THRESHOLD} threshold"
        )
    ...
    for pred in gnn_predictions:
        if (
            pred.get("prune_recommended", False)
            and pred.get("confidence", 0) >= MIN_CONFIDENCE_THRESHOLD
        ):
    ...
    for i, leaf in enumerate(leaves):
        ...
        is_proof_path = leaf.get("is_proof_path", False) or leaf.get(
            "audit_path", False
        )
    ...
    if proof_paths_seen < MIN_PROOF_PATHS_RETAINED:
        ...
        for i, leaf in enumerate(leaves):
            if (
                len(pruned_leaves)
                >= original_count - branches_pruned + additional_needed
            ):
                break
